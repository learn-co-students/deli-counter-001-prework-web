...
def line(katz_deli)
  if katz_deli.size == 0 #if the array size is zero then it needs to output the phrase below
    ...
    #need to identify the name and index position. Using each_with_index because needs to pull both of these items. Block assigns the array items to be pulls in the enumeration. Need to add       each array item to the string line_list starting with the index [position + 1] so it starts at one rather than zero, then a period, then the name
